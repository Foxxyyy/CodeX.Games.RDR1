...
namespace CodeX.Games.RDR1.RPF6
{
    public class Rpf6FileManager : FileManager
    {
        ...
        public override void InitFileTypes()
        {
            ...
            InitFileType(".wsi", "Sector Info", FileTypeIcon.Process, FileTypeAction.ViewXml, false, false, true);
        ...
        private void InitGameFiles()
        {
            ...
            if (DataFileMgr != null)
            {
                if (DataFileMgr.UseStartupCache == false)
                {
                    DataFileMgr = null;
                }
            }
        ...
        {
            //TODO: load dependencies properly instead of brute force loading everything
            //TODO: use Rsc6Drawable.ApplyTextures
            ...
            foreach (var piece in pack.Pieces.Values)
            {
                ...
                Parallel.ForEach(allMeshes, mesh =>
                {
                    ...
                    for (int i = 0; i < mesh.Textures.Length; i++)
                    {
                        ...
                        var texName = texture.Name.ToLowerInvariant().Replace(".dds", "");
                        var tex = texturesDict.Values.FirstOrDefault(t => t.Name.Equals(texName, StringComparison.InvariantCultureIgnoreCase));
                        ...
                        foreach (var shader in ((Rsc6Drawable)piece)?.ShaderGroup.Item?.Shaders.Items)
                        {
                            foreach (var param in shader?.ParametersList.Item?.Parameters)
                            {
                                ...
                                if (tex.Name.Equals(param.Texture.Name, StringComparison.InvariantCultureIgnoreCase))
                                {
                        ...
                        foreach (var mesh in model.Meshes)
                        {
                            foreach (var texture in mesh.Textures)
                            {
                                ...
                                foreach (var wtd in wtdFiles)
                                {
                                    ...
                                    if (!wtd.Value.NameLower.StartsWith(texture.Name)) continue;
                ...
                foreach (var kv in dfm.SmicMapping)
                {
                    ...
                    foreach (var smic in kv.Value)
                    {
                        if (dfm.StreamEntries[Rpf6FileExt.wtd_wtx].TryGetValue(JenkHash.GenHash(smic.ToLower()), out Rpf6FileEntry smicFile))
                        {
                            ...
                            {
                                if (tex == null) continue;
                                if (textures.Find(item => item.Name.Equals(tex.Name, StringComparison.InvariantCultureIgnoreCase)) == null)
                                {
                                    textures.Add((Rsc6Texture)tex);
                                }
                            }
            ...
            foreach (var archive in AllArchives)
            {
                ...
                foreach (var entry in archive.AllEntries)
                {
                    if (entry is Rpf6FileEntry fe)
                    {
                        ...
                        if (strtable && n.EndsWith(".strtbl") && archive.Name == "strings_d11generic.rpf")
                        {
    ...
    public class Rpf6DataFileMgr
    {
        ...
        public void Init()
        {
            ...
            SwAll = new List<Rsc6Texture>();
            FragTextures = new List<Rsc6Texture>();
            SmicMapping = new Dictionary<string, string[]>();
        ...
        private void LoadResidentTextures()
        {
            ...
            if (swall != null)
            {
                ...
                if (r != null)
                {
                    ...
                    {
                        if (tex == null) continue;
                        if (SwAll.Find(item => item.Name.Equals(tex.Name, StringComparison.InvariantCultureIgnoreCase)) == null)
                        {
            ...
            if (texList != null)
            {
                ...
                if (r != null)
                {
                    ...
                    {
                        if (tex == null) continue;
                        if (FragTextures.Find(item => item.Name.Equals(tex.Name, StringComparison.InvariantCultureIgnoreCase)) == null)
                        {
